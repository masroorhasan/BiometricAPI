 ...
 */
angular.module('vcui.services', []).factory('AuthService', ['$cookieStore', '$http', '$location', '$log', 'VCUI_CONFIG', function($cookieStore, $http, $location, $log, VCUI_CONFIG) {
  ...
  return {
    ...
    sendLogin: function(username, password, callback) {
      $http.post(VCUI_CONFIG.api_hostname + 'auth/', {
        'username': username,
        'password': password
    ...
    isLoggedIn: function() {
      var self = this;
      if ((self.username && self.sessionID) || VCUI_CONFIG.debug || $location.search()['static']) {
        return true;
      } else {
        return false;
      }
